...
function toMetricArray(input: unknown): Metric[] {
    if (Array.isArray(input)) return input as Metric[];
    if (input && typeof input === 'object' && 'data' in (input as Record<string, unknown>)) {
        const d = (input as { data?: unknown }).data;
        if (Array.isArray(d)) return d as Metric[];
    }
    return [];
}
...
export default async function TestRunPage({ params }: PageProps) {
    try {
        ...
        while (true) {
            ...
            // Support both plain array and paginated object shapes
            const pageData = ((): { items: TestResultDetail[]; total?: number } => {
                ...
                if (raw && typeof raw === 'object') {
                    ...
                    const total =
                        obj.pagination?.totalCount ?? obj.total ?? undefined;
                    return { items, total };
                }
                return { items: [] };
            })();

            allResults = allResults.concat(pageData.items);
            if (pageData.total !== undefined) {
                totalCount = pageData.total;
                if (allResults.length >= totalCount) break;
            } else {
                if (pageData.items.length < batchSize) break;
            }
        ...
        const promptsMap = promptSettled.reduce<Record<string, Prompt>>(
            (acc, result, idx) => {
                ...
                if (result.status === 'fulfilled') {
                    const prompt = result.value.data;
        ...
        // Filter to only behaviors with ids to avoid undefined in Promise.all
        const behaviorList = rawBehaviors?.filter(
            (b): b is Behavior & { id: string } => Boolean(b && b.id),
        ) ?? [];

        const withMetrics = await Promise.all(
            behaviorList.map(async (b): Promise<Behavior & { metrics: Metric[] }> => {
                try {
                    ...
                    const metricsArray = toMetricArray(metricsRes);
        ...
        const behaviors: Array<Behavior & { metrics: Metric[] }> = withMetrics.map((b) => ({
            ...b,
            metrics: b.metrics ?? [],
        }));
            ...
            <PageContainer title={title} breadcrumbs={breadcrumbs}>
                <Box sx={{ flexGrow: 1, pt: 3 }}>
                    <TestRunMainView
                        ...
                        testRun={testRun as TestRunDetail}
                        testResults={allResults}
                        prompts={
                            (promptsMap as unknown) as Record<string, { content: string; name?: string }>
                        }
                        behaviors={
                            behaviors.map((b) => ({
                                id: b.id,
