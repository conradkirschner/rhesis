// src/components/tasks/TasksSection.tsx
'use client';
...
import { EntityType, TaskCreate, TaskDetail } from '@/api-client';
...
export function TasksSection({
    ...
    const tasks: TaskDetail[] = payload?.data ?? [];
    const totalCount: number = payload?.pagination?.totalCount ?? tasks.length;

    const loading = listQuery.isLoading;
    const errorMsg = listQuery.isError
        ? (listQuery.error as Error)?.message ?? 'Failed to fetch tasks'
        : null;
    ...
    const columns: GridColDef<TaskDetail>[] = [
        {
            field: 'title',
            headerName: 'Title',
            width: 300,
        ...
        },
        {
            ...
            renderCell: (params: GridRenderCellParams<TaskDetail>) => (
                <Chip
                    ...
                    color={getStatusColor(params.row.status?.name ?? undefined)}
                    size="small"
                    variant="outlined"
                />
            ),
            ...
            <Box>
                ...
                {errorMsg ? (
                    ...
                    <BaseDataGrid<TaskDetail>
                        ...
                        getRowId={(row: TaskDetail) => String(row.id)}
                        showToolbar
                        disablePaperWrapper
