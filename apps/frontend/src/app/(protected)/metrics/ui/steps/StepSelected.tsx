// apps/frontend/src/app/(protected)/metrics/ui/steps/StepSelected.tsx
'use client';
...
export default function StepSelected({
    ...
    const [editing, setEditing] = React.useState<EditingDraft | null>(null);
    const isNew = !editing?.id;

    const openCreate = () => {
        setEditing({ name: '', description: '' }); // keep empty string allowed
        setDrawerOpen(true);
    };
    ...
    // ensure updates always return a full EditingDraft (no undefined fields)
    const updateEditing = React.useCallback(
        <K extends keyof EditingDraft>(key: K, value: EditingDraft[K]) => {
            setEditing((prev) => {
                const base: EditingDraft = {
                    id: prev?.id,
                    name: prev?.name ?? '',
                ...
                <DialogContent>
                    <Stack spacing={2} sx={{ mt: 1 }}>
                        ...
                        <TextField
                            ...
                            multiline
                            rows={4}
                            data-test-id="dimension-description"
                        />
                        {!isNew ? (
                            <>
                                <Divider />
                ...
                >
                    {section.metrics.map((metric) => (
                        <MetricTile
                            key={metric.id}
                            metric={metric}
                            onOpenDetail={() => onOpenDetail(metric.id)}
