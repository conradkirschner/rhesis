...
import { UiTestRow, UiCreateTestFormData } from '../ui/types';
import StepSummaryCharts from '../../ui/steps/StepSummaryCharts';
import StepTestsTable from '../../ui/steps/StepTestsTable';

type PageCrumb = { readonly title: string; readonly path: string };
type HeaderCrumb = { readonly href: string; readonly label: string };
...
export default function TestsContainer() {
  ...
  const pageCrumbs: readonly PageCrumb[] = useMemo(
      () => [{ title: 'Tests', path: '/tests' }] as const,
      [],
  );
  const headerCrumbs: readonly HeaderCrumb[] = useMemo(
      () => [{ href: '/tests', label: 'Tests' }] as const,
      [],
  );
  ...
  const handleAssociate = useCallback(
      async (testSetId: string, ids: readonly string[]) => {
        try {
  ...
  const handleCreate = useCallback(
      async (data: UiCreateTestFormData) => {
        try {
  ...
  if (status === 'loading') {
    return (
        <FeaturePageFrame title="Tests" breadcrumbs={pageCrumbs}>
          <InlineLoader />
        </FeaturePageFrame>
    );
  }
  ...
  return (
      <FeaturePageFrame title="Tests" breadcrumbs={pageCrumbs}>
        <StepperHeader title="Overview" subtitle="Recent distribution" breadcrumbs={headerCrumbs} />
        <StepSummaryCharts
        ...
        <StepTestsTable
            ...
            page={page}
            pageSize={pageSize}
            onPaginationChange={(next: { page: number; pageSize: number }) => {
              setPage(next.page);
              setPageSize(next.pageSize);
            }}
            onFilterODataChange={(odata: string | null) => {
              setPage(0);
            ...
            onAssociateSelected={(testSetId: string) => handleAssociate(testSetId, selectedIds)}
