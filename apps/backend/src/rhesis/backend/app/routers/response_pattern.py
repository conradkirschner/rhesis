...
@router.get("/", response_model=Paginated[schemas.ResponsePattern])
@with_count_header(model=models.ResponsePattern, to_body=True)
def read_response_patterns(
    ...
    tenant_context=Depends(get_tenant_context),
    current_user: User = Depends(require_current_user_or_token)
) -> Paginated[schemas.ResponsePattern]:
...
def read_response_pattern(
    ...
    db_response_pattern = crud.get_response_pattern(
        db, response_pattern_id=response_pattern_id, organization_id=organization_id, user_id=user_id
    )
