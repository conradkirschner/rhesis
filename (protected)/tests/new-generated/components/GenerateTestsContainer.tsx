...
import { useGenerateTestsData } from '@/src/hooks/data/GenerateTests/useGenerateTestsData';
import type {
  UiActionBarProps,
...
import { toPromptFromUiConfig } from '@/src/lib/generate-tests/format';

const INITIAL_CONFIG: UiTestSetGenerationConfig = {
  project_name: null,
  behaviors: [],
  purposes: [],
...
const STEP_LABELS: UiStepLabels = ['Configure Generation', 'Upload Documents', 'Review Samples', 'Confirm & Generate'] as const;
...
export default function GenerateTestsContainer() {
  ...
  const [configData, setConfigData] = useState<UiTestSetGenerationConfig>(INITIAL_CONFIG);
  ...
  const onFilesSelected = useCallback<UiConfirmGenerateProps['onFilesSelected']>(
    async (files) => {
  ...
  const onDocumentUpdate = useCallback<UiConfirmGenerateProps['onDocumentUpdate']>((id, field, value) => {
    setDocuments((prev) => prev.map((d) => (d.id === id ? { ...d, [field]: value } : d)));
  }, []);

  const onDocumentRemove = useCallback<UiConfirmGenerateProps['onDocumentRemove']>((id) => {
  ...
  const handleNext = useCallback<UiActionBarProps['onNext']>(async () => {
    if (activeStep === 1) {
      ...
      try {
        const docsPayload = documents.filter((d) => d.status === 'completed').map((d) => ({
          name: d.name,
          description: d.description,
          content: d.content,
        }));
        const initial = await generateSamples(configData, 5, docsPayload);
        setSamples(initial);
  ...
  const handleLoadMore = useCallback<UiReviewSamplesProps['onLoadMore']>(async () => {
    try {
      ...
      const more = await loadMoreSamples(start, configData, docsPayload, 5);
  ...
  const handleFinish = useCallback<UiActionBarProps['onFinish']>(async () => {
    setIsFinishing(true);
    try {
      const message = await createTestSet(
        {
  ...
  const stepProps = useMemo(() => {
    ...
    return {
      configure: {
        ...
        toPromptPreview: () => toPromptFromUiConfig(configData),
      ...
      confirm: {
        ...
        averageRating:
          samples.filter((s) => s.rating !== null).length > 0
            ...
            : 'N/A',
        promptPreview: toPromptFromUiConfig(configData),
        docsPayload,
  ...
  const actionBarProps = useMemo(
    () =>
      ({
        ...
        nextDisabled:
          isGenerating ||
          (activeStep === 1 && documents.some((d) => d.status !== 'completed' && d.status !== 'error')),
